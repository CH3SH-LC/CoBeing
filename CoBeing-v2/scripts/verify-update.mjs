...
async function main() {
  ...
  record(
    '电脑端挑选正式版 setup.exe',
    !!desktopPick && desktopPick.rel.tag_name === 'v2.0.7' && desktopPick.asset.name === 'CoBeing.v2_2.0.7_x64-setup.exe',
  ...
  record(
    '手机端挑选正式版 APK',
    !!mobilePick && mobilePick.rel.tag_name === 'v2.0.7' && mobilePick.asset.name === 'CoBeing-mobile-v2.0.7-debug.apk',
    mobilePick ? `${mobilePick.rel.tag_name} → ${mobilePick.asset.name}` : '未找到',
  )

  // ---- 版本比较 ----
  const latestTag = desktopPick?.rel.tag_name ?? 'v2.0.7'
  record('当前 2.0.7 已是最新（无更新提示）', !isNewerVersion(latestTag, CURRENT), `${latestTag} vs ${CURRENT}`)
