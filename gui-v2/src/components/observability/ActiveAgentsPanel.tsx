...
export function ActiveAgentsPanel() {
  ...
  const nonGroupActive = activeAgents.filter(a =>
    !groupIds.some(gid => queues[gid].processing === a.agentId)
  );
  const hasAny = totalQueued > 0 || totalProcessing > 0 || nonGroupActive.length > 0;
    ...
    <div className="flex flex-col rounded-xl bg-surface border border-bdr/40" style={{ padding: 18, boxShadow: "var(--shadow-surface)", minHeight: 200 }}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-txt-muted">
          {hasAny
            ? `队列 ${totalQueued} · 执行中 ${totalProcessing + nonGroupActive.length}`
            : "空闲"}
        </span>
      </div>
      ...
      {!hasAny ? (
        ...
        <div className="flex flex-col overflow-y-auto" style={{ gap: 6, maxHeight: 300 }}>
          {/* 非群组活跃 Agent（直接对话/TODO触发） */}
          {nonGroupActive.map((a) => (
          ...
          {groupIds.map((gid) => {
            ...
            return (
              <div key={gid} className="flex flex-col" style={{ gap: 4 }}>
                {gq.processing && (
                  <ProcessingItem agentId={gq.processing} source={`群组 ${gq.groupName}`} />
                )}
