  ...
  return (
    <div className="rounded-xl bg-surface border border-bdr/40"
      ...
      <div className="flex gap-6 mt-3">
        <div><p className="text-xs text-txt-muted">P50</p><p className="text-xl font-bold text-accent">{(data.latency.p50 / 1000).toFixed(1)}s</p></div>
        <div><p className="text-xs text-txt-muted">P95</p><p className="text-xl font-bold text-txt">{(data.latency.p95 / 1000).toFixed(1)}s</p></div>
      </div>
      {hourly.length > 0 && (
