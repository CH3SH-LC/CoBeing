...
const FIELD_DEFS: Record<string, { label: string; hint: string; placeholder: string; required?: boolean; sensitive?: boolean }> = {
...
export function ChannelsSection() {
  ...
  const pluginChannels = usePluginsStore((s) => s.channels);
  ...
  const entries = useMemo(() => {
    ...
    for (const [key, ch] of Object.entries(channels).filter(([_key, ch]) => ch != null)) {
      result.push([key, ch]);
      added.add(key);
    }
  ...
  const confirmDelete = () => {
    if (deleteTarget) {
      getWsClient()?.send({
        type: "unbind_channel",
        payload: { channelName: deleteTarget },
      });
          ...
          {entries.map(([key, ch]) => {
            ...
            return (
              <div key={key} className="flex items-center gap-3 rounded-lg bg-elevated" style={{ padding: "14px 20px" }}>
                ...
                {ch._pluginManaged && (
                  <span className="text-xs text-txt-muted">由插件管理</span>
                )}
              </div>
            );
          })}
