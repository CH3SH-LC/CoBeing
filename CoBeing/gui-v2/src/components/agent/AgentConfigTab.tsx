      ...
      <div className="rounded-xl bg-elevated" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {sandboxEnabled && (
          <>
            {/* 网络模式选择 */}
            <div>
              <label className="text-sm text-txt-sub mb-1.5 block">网络模式</label>
              ...
              <div className="rounded-xl bg-elevated border border-bdr/30" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {allowDomains.map((domain, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <button onClick={() => { setAllowDomains(prev => prev.filter((_, idx) => idx !== i)); setSaved(false); }}
                      className="text-sm text-danger hover:text-danger/80">删除</button>
                  </div>
                ))}
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { if (newDomain) { setAllowDomains(prev => [...prev, newDomain]); setNewDomain(""); setSaved(false); } }}
                    className="h-9 px-4 rounded-lg bg-accent text-white text-sm font-medium">添加</button>
                </div>
                <div className="flex flex-wrap gap-1 mt-2">
                  {["dev-tools", "package-managers", "documentation"].map(groupId => (
                    <button key={groupId} onClick={() => {
                      ...
                      setSaved(false);
                    }} className={`px-3 py-1.5 rounded-lg text-sm ${selectedGroups.includes(groupId) ? "bg-accent text-white" : "bg-hover text-txt-sub"}`}>
                      {groupId}
                    </button>
                  ))}
                </div>
            ...
            <div className="rounded-xl bg-elevated border border-bdr/30" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {mounts.map((mount, i) => (
                <div key={i} className="flex items-center gap-2 text-sm">
                  ...
                  <label className="flex items-center gap-1 text-sm text-txt-sub">
                  ...
                  <button onClick={() => { setMounts(prev => prev.filter((_, idx) => idx !== i)); setSaved(false); }}
                    className="text-sm text-danger">删除</button>
                </div>
              ))}
              <button onClick={() => {
              ...
              }} className="h-9 px-4 rounded-lg bg-accent text-white text-sm font-medium">添加挂载</button>
            </div>

            {/* 资源限制 */}
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="text-sm text-txt-sub mb-1.5 block">内存限制</label>
              ...
              <div>
                <label className="text-sm text-txt-sub mb-1.5 block">CPU 核数</label>
              ...
              <div>
                <label className="text-sm text-txt-sub mb-1.5 block">超时(秒)</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-txt-sub mb-1.5 block">磁盘限制</label>
              ...
              <div>
                <label className="text-sm text-txt-sub mb-1.5 block">镜像</label>
