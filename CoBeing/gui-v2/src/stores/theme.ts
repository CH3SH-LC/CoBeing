...
const REQUIRED_PATHS = [
  ...
  "chat.msg-user", "chat.msg-assistant", "chat.msg-system", "chat.msg-tool",
];
...
export async function fetchBuiltInIds(): Promise<string[]> {
  try {
    const resp = await fetch("/themes/manifest.json", { cache: "no-store" });
...
export async function loadBuiltInTheme(id: string): Promise<ThemePreset> {
  const resp = await fetch(`/themes/${id}.json`, { cache: "no-store" });
  return await resp.json() as ThemePreset;
}

export function mergeThemePresets(
  builtIn: Record<string, ThemePreset>,
  custom: Record<string, ThemePreset>,
  ...
  loadThemes: async () => {
    try {
      ...
      for (const id of themeIds) {
        try {
          builtIn[id] = await loadBuiltInTheme(id);
        } catch {
      ...
      const all = mergeThemePresets(builtIn, custom);
