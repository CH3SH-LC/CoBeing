 ...
 *
 * 与 claude-code/office/qqbot 包一致的手写精简 MCP 服务端，通过 stdin/stdout 与
 * CoBeing 的 MCPClient 通信。每个 JSON-RPC 消息为一行 JSON。
 * initialize 返回 instructions（登录态信任边界等使用指南），随 mcp-register 一并提供。
 */
...
interface ServerConfig {
  ...
  /** 使用指南：Agent 注册本 server 工具时随 mcp-register 一并提供 */
  instructions?: string;
  tools: Tool[];
}
