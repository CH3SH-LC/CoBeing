...
const FALLBACK_SYSTEM_PROMPT = `你是群组中的判断助手。唯一职责：审查 Agent 发言，决定是否需要唤醒群主。
...
function getJudgmentPrompt(): string {
  const { prompt } = loadToolAgentData("judgment");
    ...
    const result = await runToolAgent(
      {
        ...
        maxIterations: 2,
        tools: [],
        systemPrompt: getJudgmentPrompt(),
        userPrompt,
        workingDir,
