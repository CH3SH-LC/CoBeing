...
describe("classifyBash", () => {
  ...
  it("denies shell chaining and redirection in read-only mode", () => {
    expect(classifyBash({ command: "cat file.txt; rm file.txt", workingDirs: WD, level: "read-only" }).allowed).toBe(false);
    expect(classifyBash({ command: "ls && curl http://example.com", workingDirs: WD, level: "read-only" }).allowed).toBe(false);
    expect(classifyBash({ command: "echo hello > out.txt", workingDirs: WD, level: "read-only" }).allowed).toBe(false);
    expect(classifyBash({ command: "cat file.txt | bash", workingDirs: WD, level: "read-only" }).allowed).toBe(false);
  });
