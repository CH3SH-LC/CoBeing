...
describe("PermissionEnforcer — 5-level", () => {
  ...
  it("read-only denies MCP tools unless explicitly allowed", () => {
    const e = new PermissionEnforcer({ mode: "read-only" }, TOOL_CONFIG, WS);
    expect(e.check("mcp:server:tool", {}).allowed).toBe(false);

    const allowed = new PermissionEnforcer({ mode: "read-only", allow: ["mcp:server:tool"] }, TOOL_CONFIG, WS);
    expect(allowed.check("mcp:server:tool", {}).allowed).toBe(true);
  });
  ...
  it("workspace-readwrite checks MCP tool file paths against workspace", () => {
    const e = new PermissionEnforcer({ mode: "workspace-readwrite" }, TOOL_CONFIG, WS);
    expect(e.check("mcp:server:write", { path: `${WS}/out.txt` }).allowed).toBe(true);
    expect(e.check("mcp:server:write", { path: "../other/file.txt" }).allowed).toBe(false);
  });
