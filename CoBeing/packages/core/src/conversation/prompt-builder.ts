...
- **写文件并发纪律**：与其他成员写同一文件时，先 read-file 获取 [file-version: X] 值，写时带 baseVersion 参数。若收到「文件已被其他成员修改」的拒绝，说明有成员在同时改此文件——重新 read-file 基于最新版本续写，不要把各自修改静默覆盖。**同一文件避免多人同时改**：分工时尽量写各自独立文件（如不同章节/组件），只在确实需要共享交付物时才共写并配合版本检查。
