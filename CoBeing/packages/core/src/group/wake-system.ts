...
优先级: ${result.priority}${result.summary ? `\n摘要: ${result.summary}` : ""}`;
  ...
  private _tickQueue(): void {
    ...
    // 检查 Agent 是否已在本群组的同一 session 中运行（允许多群组/独立并发）
    const agent = this.getAgent(entry.targetAgentId);
    if (agent) {
      const groupSessionKey = `group:${this.ctx.groupId}`;
      const activeSessions = typeof agent.getActiveSessions === "function" ? agent.getActiveSessions() : [];
      const busyInThisGroup = activeSessions.includes(groupSessionKey);
      if (busyInThisGroup) {
        const requeueCount = ((entry as any).__requeueCount ?? 0) + 1;
        const MAX_REQUEUE = 10;
        if (requeueCount > MAX_REQUEUE) {
          log.warn("[%s] Tick: dropping wake for %s after %d requeues (stuck in same group session)", this.ctx.groupId, entry.targetAgentId, MAX_REQUEUE);
          this._broadcastQueue();
          return;
        }
        (entry as any).__requeueCount = requeueCount;
        log.info("[%s] Tick: %s busy in same group session, requeue #%d", this.ctx.groupId, entry.targetAgentId, requeueCount);
        this.wakeQueue.push(entry);
        this._broadcastQueue();
        return;
      }
    ...
    const task = this.executeWake(entry).catch(err => {
