...
describe("host-manage-todo", () => {
  ...
  it("awaits async complete callbacks before reporting success", async () => {
    const updateTodo = vi.fn().mockResolvedValue(undefined);
    const tool = makeHostManageTodoTool(() => [], updateTodo);
    const result = await tool.execute(
      { action: "complete", groupId: "g1", todoId: "missing" },
      { agentId: "host" } as any,
    );
    expect(updateTodo).toHaveBeenCalledWith("missing", { status: "completed", completedAt: expect.any(String) });
    expect(result.content).toBe("未找到 TODO");
  });
