...
export class Group {
  ...
  private resolveMention(mention: string): string | undefined {
    // 1. 精确 ID 匹配
    if (this.registry.get(mention) && this.config.members.includes(mention)) return mention;
    ...
    for (const agent of this.registry.list()) {
      ...
      if (this.config.members.includes(agent.id) && (nameNorm === normalized || agent.name.toLowerCase() === mention.toLowerCase())) {
        return agent.id;
      }
    }
  ...
  createTalk(members: string[], topic: string): string {
    const talkId = this.ctxV2.createTalk(members, topic);
    const talk = this.ctxV2.getTalk(talkId);
    if (talk && this._groupManager) {
      this._groupManager.appendTalkMeta(this.id, talk);
    }
    return talkId;
  }
  ...
  addMember(agentId: string): void {
    if (!this.config.members.includes(agentId)) {
      ...
      // Emit group:addMember hook
      const hookBus = (globalThis as any).__cobeingHookBus;
      if (hookBus) {
        const agentName = this.resolveAgentName(agentId);
        hookBus.emit("group:addMember", this.id, agentId, agentName).catch(() => {});
      }
    }
  }

  removeMember(agentId: string): void {
    // Emit group:removeMember hook
    const hookBus = (globalThis as any).__cobeingHookBus;
    if (hookBus) {
      const agentName = this.resolveAgentName(agentId);
      hookBus.emit("group:removeMember", this.id, agentId, agentName).catch(() => {});
    }
  ...
  dispose(): void {
    ...
    // Close all per-agent SQLite databases WITHOUT touching aux files
    // (Windows: touching WAL/SHM post-close can trigger native crashes)
