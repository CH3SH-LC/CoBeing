...
export function makeHostManageTodoTool(
  ...
  updateTodo?: (todoId: string, updates: any) => any | Promise<any>,
    ...
    async execute(params, _context: ToolContext): Promise<ToolResult> {
      ...
      switch (action) {
        ...
        case "assign": {
          ...
          const updated = await updateTodo(params.todoId as string, { targetAgentId: params.assignee });
          return { toolCallId: "", content: updated ? `已分配 TODO ${params.todoId} 给 ${params.assignee}` : "未找到 TODO" };
        }
        case "complete": {
          if (!updateTodo) return { toolCallId: "", content: "updateTodo 未配置", isError: true };
          const completed = await updateTodo(params.todoId as string, { status: "completed", completedAt: new Date().toISOString() });
