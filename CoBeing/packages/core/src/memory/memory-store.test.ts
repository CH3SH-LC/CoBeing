...
describe("MemoryStore — read", () => {
  it("reads single target", () => {
    store.add("experience", "learned about dark mode preferences");
    const result = store.read("experience");
    expect(result.success).toBe(true);
  ...
  it("reads all targets", () => {
    ...
    store.add("experience", "experience content");
    const result = store.read();
    expect(result.content).toContain("mem content");
    expect(result.content).toContain("experience content");
  });
...
describe("MemoryStore — snapshot", () => {
  ...
  it("returns empty for empty target", () => {
    const block = store.formatForSystemPrompt("experience");
    expect(block).toBe("");
  });
