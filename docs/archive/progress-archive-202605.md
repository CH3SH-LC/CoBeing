# CoBeing 开发进度记录（归档：2026-05 及更早）

> 此文件为 PROGRESS.md 的历史归档，包含 2026-05 及更早的进度条目。
> 当前活跃进度见 PROGRESS.md。
...
### 第五轮：预启动残留清理机制 — 彻底解决删除文件残留

**问题**：第四轮在删除路径中添加 `cleanupSQLiteAuxFiles` 仍无法可靠清理残留目录。根因是 Windows 上 better-sqlite3 原生模块关闭后文件锁可能延迟释放，且 `cleanupOrphanDirectories()` 在 `restoreAgents()`/`restoreGroups()` 之后调用，此时 SQLite 连接已全部打开，删除被锁文件必然失败。

**核心思路转变**：不在删除时强制清理（运行时 SQLite 持有文件锁），改为：
1. 删除时：`rename` 目录为 `.deleted.{timestamp}` 标记（目录项操作不受文件锁影响）
2. 启动时：在任何 SQLite 连接建立**之前**扫描并 `rmDirForce` 所有标记目录

**修改文件**：
- `packages/shared/src/fs-utils.ts` — 新增 `cleanupPendingDeletions(dataRoot)` 导出函数，扫描 `data/agents/` 和 `data/groups/` 下匹配 `*.deleted.*` / `*.orphan.*` / `*.pending-delete.*` 的目录和文件并强制删除
- `packages/core/src/runtime.ts` — 构造函数第一行调用 `cleanupPendingDeletions(this.dataRoot)`，在 `new GroupManager` / `new ObservabilityDB` 等任何 SQLite 连接之前执行；导入新增 `cleanupPendingDeletions`
- `packages/core/src/agent/butler.ts` — `butler-destroy-agent` 删除路径新增 config.json rename 兜底（对齐 ws-server.ts 和 manager.ts 的已有防护）
- `start.bat` — 新增预启动 PowerShell 清理步骤，在 Node.js 进程启动前调用 `scripts/cleanup-residues.ps1` 删除残留目录/文件
- `scripts/cleanup-residues.ps1` — [新建] 预启动残留清理脚本，遍历 `data/agents/` 和 `data/groups/` 删除标记目录和文件

**三层防护**：
1. `start.bat` → PowerShell 脚本（Node 启动前）
2. `CoBeingRuntime` 构造函数第一行（SQLite 连接前）
3. `cleanupOrphanDirectories()` 中的 `.deleted.` 清理（防御纵深）

**验证**：pnpm build 7pkgs pass, pnpm test 417 pass (43 files)

---

### 第四轮：删除文件残留 + Agent 超时 + WS 离线深度修复

**删除群组/Agent 文件残留修复**

**问题**：管家删除或手动删除群组/Agent 时，只删注册表，实际文件目录残留。

**根因**：SQLite WAL 模式下 `close()` 后 Windows 可能短暂锁定 `-wal` / `-shm` 辅助文件。`MemoryStore.close()` 原来只关连接不清理磁盘文件。删除路径中 `rmDirRecursive` 尝试删除被锁文件失败 → 目录无法删除 → 残留。

**修改文件**：
- `packages/shared/src/fs-utils.ts` — 无需改动（`cleanupSQLiteAuxFiles` 已存在，具备 unlink + rename 双兜底）
- `packages/core/src/memory/memory-store.ts` — `close()` 新增 `cleanupSQLiteAuxFiles("memory.db")`；导入 `cleanupSQLiteAuxFiles`
- `packages/core/src/group/group-db.ts` — 新增 `get databasePath()` 公开 getter
- `packages/core/src/group/manager.ts` — `delete()` 删目录前显式 `cleanupSQLiteAuxFiles("group.db")`；导入
- `packages/core/src/api/ws-server.ts` — `destroy_agent` 删目录前 `cleanupSQLiteAuxFiles`；导入
- `packages/core/src/agent/butler.ts` — `butler-destroy-agent` 删目录前 `cleanupSQLiteAuxFiles`；导入

**Agent 执行超时保护**

**问题**：Agent LLM 调用挂起时 WakeSystem 永久阻塞，`onAgentResponse` 永不触发，前端收不到回复。

**修改文件**：
- `packages/core/src/agent/agent.ts` — `run()` 新增 5 分钟兜底超时（`setTimeout(300s) → ac.abort()`），catch 块检测 AbortError 返回取消消息，finally 清理 timer

**WS 离线窗口冻结深度修复（第二轮）**

**问题**：前两轮修复后窗口仍变灰/离线/无法操作。

**修改文件**：
- `gui-v2/src/lib/ws-client.ts` — 完整重写：5s 离线宽限期（期内重连不展示离线）、客户端 pong 超时检测（5s 无响应→主动 close 触发重连）、`_ping` 心跳不排队（避免占速率预算）、重连加速（500ms 初始→10s 上限）、`_connected` 始终广播确保状态加载
- `gui-v2/src/components/chat/ChatView.tsx` — `canSend` 移除 `connected` 条件（离线时仍可输入，消息排队）
- `gui-v2/src/components/chat/GroupChatView.tsx` — 同上
- `packages/core/src/api/ws-server.ts` — pong 超时 10s → 20s

---

### 第三轮：审计修复（4 项）

**变更原因**：对前两轮变更进行系统性代码审计，发现 4 个确认 bug。

**修改文件**：
- `packages/core/src/agent/agent.ts` — C1: `_abortController` 单字段 → `Map<string, AbortController>` per-session，`stop()` 遍历 abort 全部；C2: 新增 `_errorFlag`，`getStatus()` 恢复 "error" 返回，catch 设置、新 run 清除
- `gui-v2/src/stores/wakeQueue.ts` — M1: `updateQueue()` 签名增加 `processingAgents` 参数，实现中保留现有值
- `gui-v2/src/hooks/useWebSocket.ts` — M1: WS handler 透传 `processingAgents` 给 `updateQueue`
- `gui-v2/src/lib/ws-client.ts` — L1: Tauri `onFocusChanged` 监听用 `tauriRef: { unbind }` 对象包装器，避免异步 resolve 时解绑失效

**验证**：pnpm build 7pkgs pass, pnpm test 417 pass (43 files)

---

### 第二轮：Agent 并发架构重构 + WS 后台稳定性 + 仪表盘来源修复

**变更原因**：用户反馈三个深度问题：
1. 程序在后台失焦状态下不稳定，需要保证稳定工作
2. Agent 应该能同时在多群组+独立工作中并发执行，而非全局互斥
3. 前端仪表盘活跃 Agent 面板中，群组内被触发的 Agent 错误显示为"独立任务"

**根因分析**：

1. **后台稳定性**：WebView2 在 Tauri 窗口失焦时会对主线程 JavaScript 执行节流（timer throttling）和进程降级（CalculateNativeWinOcclusion），导致 WebSocket 心跳中断。之前的前端 visibility API 修复只能在恢复焦点时被动重连，无法主动预防断连。

2. **Agent 并发**：`agent.ts` 使用全局 `_status: "idle" | "running"` 互斥锁，导致 Agent 在直接对话中忙碌时无法响应群组 @mention，在群组 A 中忙碌时无法在群组 B 中被唤醒。实际上 `sessionLoops` Map 早已支持 per-session 的 ConversationLoop 隔离，全局锁是唯一的阻塞点。

3. **仪表盘来源错误**：`get_wake_queue` 收集 `activeAgents` 时未提取 `groupId`（只传了 `agentId/agentName/status`），`agent.getStatus()` 只返回 "idle"/"running" 无 session 上下文。前端 `ActiveAgentsPanel` 通过比对 `queues[gid].processing`（单个 agent ID）来猜测来源，在并发场景下必然出错。

**修改文件**：

**后台稳定性**：
- `gui-v2/src-tauri/tauri.conf.json` — windows[0] 新增 `"additionalBrowserArgs": "--disable-background-timer-throttling --disable-features=CalculateNativeWinOcclusion,DisableProcessThrottling"`
- `gui-v2/src-tauri/src/lib.rs` — 导入 WebviewWindow；setup 钩子中对 main 窗口执行 Windows 平台的后台节流抑制
- `gui-v2/src/lib/ws-client.ts` — 心跳间隔 25s → 10s

**Agent 并发架构**：
- `packages/core/src/agent/agent.ts` — 核心重构：
  - 移除 `private _status: AgentStatus = "idle"`
  - 新增 `private _activeSessions = new Set<string>()` — per-session 活跃追踪
  - `run()`: 守卫改为 `_activeSessions.has(sessionKey)`（仅阻止同 session 并发），track/untrack 改为 add/delete
  - `handleIncomingMessage()`: 同上 per-session 守卫
  - `stop()`: 改为 `_activeSessions.clear()`（停止所有 session）
  - `getStatus()`: 改为 `_activeSessions.size > 0 ? "running" : "idle"`
  - 新增 `getActiveSessions(): string[]` — 返回活跃 session 键列表
- `packages/core/src/group/wake-system.ts` — `_tickQueue()` 忙碌检查改为仅检查同群组 session（`group:<groupId>`），允许 Agent 在 groupA+groupB+main 三线并发

**仪表盘来源修复**：
- `packages/core/src/api/ws-server.ts` — `get_wake_queue`: 从 `agent.getActiveSessions()` 提取 groupId（`"group:<id>".slice(6)`），填入 `activeAgents[].groupId`
- `gui-v2/src/hooks/useWebSocket.ts` — `wake_queue_update` payload 类型增加 `processingAgents?: string[]` 和 `activeAgents[].groupId?: string`
- `gui-v2/src/stores/wakeQueue.ts` — `GroupWakeQueue` 接口增加 `processingAgents?: string[]`
- `gui-v2/src/components/observability/ActiveAgentsPanel.tsx` — 重写来源判断逻辑：
  - 导入 `useGroupsStore` + 新增 `resolveGroupName()` 辅助函数
  - 按 `activeAgents[].groupId` 区分为 `groupActive` / `independentActive`
  - 群组活跃 Agent 显示"群组 <名称>"而非"独立任务"

**验证**：pnpm build 7pkgs pass, pnpm test 417 pass (43 files)

---

### 第一轮：WS 连接失焦断开 + Agent 群组回复失败 + 群组创建用户决策强化

**变更原因**：用户反馈三个问题：
1. 切换到其他窗口后 CoBeing 前端显示离线
2. 部分智能体在独立窗口或群组中时无法正常在群组内回复
3. 群组创建后智能体自行完成大量工作再汇报，未征求用户意见

**根因分析**：
1. WebView2 窗口失焦时可能节流 WebSocket 连接，仅依赖原生 ping/pong 不足以保活，且重连为固定 3s 无退避
2. `agent.run()` 有硬并发守卫（`_status === "running"` → 直接返回错误），WakeSystem 只检查自己的 `_processingAgents` 不检查 Agent 真实状态，导致 Agent 在直接对话或群组中忙碌时群组唤醒静默失败
3. 系统提示词中 `GROUP_MECHANICS_NOTICE`、群主职责、协作规则均鼓励自主行动，创建消息虽说"与用户对接"但被更强的系统提示掩盖

**修改文件**：
- `gui-v2/src/lib/ws-client.ts` — 新增应用层心跳(25s `_ping`)、`visibilitychange` + Tauri `onFocusChanged` 窗口恢复焦点时主动重连、指数退避重连(2s→4s→8s→16s→max 30s)、`checkAndReconnect()` 公开方法、`_pong` 消息过滤
- `packages/core/src/api/ws-server.ts` — 新增 `_ping` → `_pong` 响应 case
- `packages/core/src/group/wake-system.ts` — `_tickQueue()` 新增 `agent.getStatus()` 检查，Agent 非 idle 时重新入队（`__requeueCount` 上限 10 次防死循环）
- `packages/core/src/conversation/prompt-builder.ts` — `GROUP_MECHANICS_NOTICE` 增加"用户为上"规则；群主职责增加"首要原则：用户决策制"（4 步：复述→规划→决策→汇报）；协作规则增加"用户决策优先"；角色自适应提示增加"重大方向性决策仍需与用户确认"
...
### Change: CoBeing GUI 三层主体结构与跨界面视觉统一

**变更原因**：
用户明确了当前前端页面逻辑：顶部条暂时不变、最左侧导航栏不变；顶部条下方、最左侧导航栏右侧的主体区域需要分为“背景”底层、其上的三个卡片（左侧列表卡片、上侧标题卡片、主体对话卡片）以及最上层设置浮层。要求所有界面统一留白、层级、长宽比例和字号，并覆盖智能体设置、群组设置、新手教程和管家全局任务。
...
- `CoBeing/CLAUDE.md`
- `CoBeing/gui-v2/src/components/layout/Surface.tsx`
- `CoBeing/gui-v2/src/components/layout/MainContent.tsx`
- `CoBeing/gui-v2/src/components/layout/AppLayout.tsx`
- `CoBeing/gui-v2/src/components/layout/Sidebar.tsx`
- `CoBeing/gui-v2/src/components/chat/ChatView.tsx`
- `CoBeing/gui-v2/src/components/chat/GroupChatView.tsx`
- `CoBeing/gui-v2/src/components/todo/GlobalTodoPanel.tsx`
- `CoBeing/gui-v2/src/components/ui/sheet.tsx`
- `CoBeing/gui-v2/src/components/ui/dialog.tsx`
- `CoBeing/gui-v2/src/components/ui/button.tsx`
- `CoBeing/gui-v2/src/components/ui/tabs.tsx`
- `CoBeing/gui-v2/src/components/tutorial/TutorialOverlay.tsx`
...
- `CoBeing/gui-v2/src/components/group/GroupConfigTab.tsx`
- `CoBeing/gui-v2/public/themes/sakura-mint.json`
- `CoBeing/gui-v2/src/styles/globals.css`
- `CoBeing/gui-v2/src/stores/theme.test.ts`
- `CoBeing/gui-v2/src/components/layout/surface-style-audit.test.ts`

**修改内容摘要**：
- 新增 CoBeing UI 术语文件，明确“背景”“卡片背景”“左侧列表卡片”“标题卡片”“主体对话卡片”“设置浮层”“磨砂质感”等含义，避免把最左侧导航栏与主体列表卡片混淆。
- 新增 CoBeing UI 风格偏好文件，固化“轻松糖果色 + 清晰层次 + 桌面应用秩序感”的默认方向。
- 更新 `CLAUDE.md`，要求以后遇到相关 UI 术语优先读取术语文件与设计偏好文件。
- 新增 `SurfaceCard` / `WorkbenchLayout`，将主体页面统一为标题卡片 + 内容卡片；聊天页使用左侧列表卡片 + 标题卡片 + 主体对话卡片。
- 将 Agent/Group/Butler 主体列表收进卡片内，管家视图左侧使用全局任务卡片。
- 对话卡片内部保留消息区和输入区；消息气泡和头像继续使用主题 token、用户右侧头像、智能体左侧头像。
- 设置 Sheet、Dialog、Select 下拉、新手教程弹层统一使用主题化磨砂遮罩和 `var(--shadow-surface*)` 层级阴影。
- 智能体设置、群组设置、成员列表、文件/工作区编辑器调整为更舒展的留白、字号和比例。
- 默认 Sakura Mint 主题增强背景与卡片之间的色差，保留糖果色轻松感，并同步 CSS fallback。
- 新增 `surface-style-audit.test.ts`，防止核心界面回退到旧小字号、旧卡片 token 或 `shadow-xl/2xl`。

**验证**：
- `..\node_modules\.bin\vitest.CMD run src/components/layout/surface-style-audit.test.ts src/stores/theme.test.ts --config vite.config.ts`：2 files / 6 tests passed。
- `.\node_modules\.bin\tsc.cmd --noEmit`：通过。
- `.\node_modules\.bin\vite.cmd build`：通过；仅保留 Vite 既有的大 chunk / 动态导入 chunking 警告。
## 2026-06-12

### Debug: 全局任务显示与 Sakura Mint 背景纠偏

**问题描述**：
管家左侧全局任务卡片显示错误：任务只显示了标题、状态和部分指派信息，没有按真实 `GlobalTodoInfo` 字段展示进度摘要、下一步、说明、负责 Agent 和执行引用；取消任务被混在“已完成”区域里，但计数只计算 completed。默认 Sakura Mint 背景也偏深、偏灰，削弱了糖果色轻松感。

**根因分析**：
- `GlobalTodoPanel` 的展示逻辑没有基于 `description`、`progressSummary`、`nextAction`、`responsibleAgentId`、`executionRefs` 等真实字段建立显示模型。
- 分组逻辑把 completed 和 cancelled 放在同一个列表里，但标题与计数只使用 completed。
- 上一轮背景色为了增强对比压得过深，粉色基底 luminance 不足，视觉上偏灰。

**修改文件**：
- `CoBeing/gui-v2/src/components/todo/GlobalTodoPanel.tsx`
- `CoBeing/gui-v2/src/components/todo/GlobalTodoPanel.test.ts`
- `CoBeing/gui-v2/public/themes/sakura-mint.json`
- `CoBeing/gui-v2/src/styles/globals.css`
- `CoBeing/gui-v2/src/stores/theme.test.ts`

**修改内容摘要**：
- 新增 `buildGlobalTodoPanelModel()`，将待派发/执行中/等待用户视为活跃任务，已完成和已取消分开显示。
- 新增 `getGlobalTodoDisplayLines()`，统一输出任务正文行和 meta 行。
- 每条全局任务现在显示：进度、下一步、说明、指派、负责 Agent、执行引用和子任务数量。
- 指派对象不再截断，避免用户看不清任务派给谁。
- Sakura Mint 背景调整为更亮的 `#FFDCEA -> #D9FFF4`，降低灰感，同时保留卡片区分度。
- 补充全局任务显示模型测试和主题亮度测试。

**验证**：
- `..\node_modules\.bin\vitest.CMD run src/components/todo/GlobalTodoPanel.test.ts src/components/layout/surface-style-audit.test.ts src/stores/theme.test.ts --config vite.config.ts`：3 files / 8 tests passed。
- `.\node_modules\.bin\tsc.cmd --noEmit`：通过。
- `.\node_modules\.bin\vite.cmd build`：通过；仅保留 Vite 既有的大 chunk / 动态导入 chunking 警告。
