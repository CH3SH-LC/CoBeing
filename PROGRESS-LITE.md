...
- [Change] 全项目重构·分层分批（2026-08-11，680 tests 全绿）：①**大文件拆分**——runtime.ts 1734→864 行（拆 runtime/{plugin-loader,providers,market,channels,core-agents}.ts）、agent.ts 1127→1036（拆 agent-tools.ts/review-prompt.ts）、wake-system.ts 860→768（拆 group/wake-context.ts），runtime 方法保留薄转发测试零改动 ②**死代码清理**——删 sqlite-adapter hrr_vector 占位列（从未读取）、删 agent experienceWriter 死字段（改 AgentFiles.ensureExperienceFile 保留文件创建）、简化 experience.ts 永不命中的 CWD prompt 路径、删 data/prompts/ 旧残留 + corrupted registry 备份 ③**预算熔断**（报告P0#5）：ConversationLoop maxTotalTokens 全链路透传 + 循环内/每轮后双重检查 + 不可逆工具 tool:irreversible 审计（bash/write/edit/rm/web-fetch）④**TODO 单一真相源**（报告P1#7）：ButlerTask eventLog 追加式状态变更事件日志（create/transition/update）+ agent-task-accept 原子认领（拒他人已认领 running）⑤**群组单一推进者**（报告P1#6）：GROUP_MECHANICS_NOTICE 补写文件并发纪律（baseVersion/拒绝时重读/避免共写），保留并发执行能力 ⑥**butler_escalation 前端接线**（报告P1#8）：新增 handler 置 waiting_user + 活动通知 + ws-butler-escalation 事件

- [Change] 完成所有待办（2026-08-09，10 批次，598→641 tests 全绿 + 冒烟 19/19·25/25 + verify-extensions 8/8）：①**TODOboard P0**（决策#3）：repeat/nextTriggerAt/overduePolicy 数据模型 + getRepeatDueTodos/advanceRepeat/stale 重唤醒 + Agent 级 0time/repeat/condition 触发接线 + todo-add 支持 repeat + GUI「下次触发」标记 + internalBlocker 展示 ②**并发写防护**（决策#2）：file-version CAS（read-file 附版本行，write/edit-file baseVersion 校验拒绝过期覆盖）③**管家工具分级**（决策#1）：BUTLER_DEFAULT_TOOLS 移除 bash/edit-file/glob/grep（保留 read/write 供个人事务 md），runtime 强制过滤+修复 config，4 JOB.md 说明 ④**GUI 入口收敛**（决策#11）：NavBar 折叠「⋯」菜单 + advancedNav 开关（localStorage），常驻管家/设置 ⑤**安全分类器**（决策#10）：PermissionMode "auto" + SafetyClassifier（reasoning-blind/fail-closed/熔断/allow 缓存）+ PermissionEnforcer auto 决策链 + executor 接线 + tool:classified 审计 ⑥**ToolAgent 收敛**（决策#8）：creator 双份 prompt 合并（配置卡权威）+ ToolAgentRegistry（复活 registerToolAgent 死注册 + loadAll）⑦**记忆纪律**（决策#6）：MemoryEntry ttl/provenance 归一化 + Q1/Q2/Q3 写入纪律 + JOB.md 每周整合 ⑧**HRR Phase 2**（决策#4）：SHA-256 确定性相位向量真实现（bind/unbind/bundle/similarity 检索）+ start-core 堆自检 + start:core 命令 ⑨**Butler 闭环**：host-report-event 结构化事件桥（6 事件类型→butler_escalation 广播）+ agent-request-resource 真实落盘收件箱 + verify-extensions 脚本（HookBus/PromptLayer/UIExtension/PluginLoader/MCP 主链路 8 项全过）
- [Debug] 真实测试闭环修复（2026-08-09）：陈默完整场景首轮 26/28（TODOboard 语义断言失败 + S4b 群组产出 2 项失败），根因均为**测试脚本缺陷非系统回归**：①TODOboard 语义断言用全量正则（残留数据误判 + `/客户a|改稿/` 误伤合法工作任务）→ 改为**增量断言**（场景前快照，只验证"本次未新增用户日程/购物清单类全局任务"——语义验证强度不变且不受历史数据影响，管家行为验证通过）②S4b 用 `groups[0]` 定位群组目录（readdirSync 按 Unicode 码点排序 → 指向旧群组「内容创作小队」）→ 改为按名称匹配「品牌」；修复后 **design 场景 + 完整 all 场景 28/28 全绿**（群组真实产出客户A_VI改版需求清单/改稿要点清单，TODOboard 增量断言通过——管家未把个人事务建账，体检日程也未进全局任务）
- [Change] 目标四项推进（2026-08-09）：①**gateway 全局化**（决策 #12 补充）：llm-gateway 不绑定 provider + chatWithGateway/chatCompleteWithGateway 统一入口 + Runtime 挂 __cobeing.gateway + **17 个 LLM 调用点全量替换**（会话循环/ToolAgent/记忆/技能/工作流/筛选/续作判断）+ 新测试 10 个（排队/重试/RPM/降级）→ **598 tests 全绿** + 陈默 morning 真实链路 S1/S2/S3 全过（唯一失败为已知复用数据边界）②**文档全量整理**：核查报告 30 项修正——2 处投票残留（README/GOAL）、CHARACTER→EXPRESSION 四处、数量口径（ws-server 591 行/78 命令/14 模块/butler 222 行 9 模块/useWebSocket 110 行）、Market 状态矛盾、问卷→对话式、STRUCTURE 补 information/、头部日期统一 ③**4 项自主调研**（spec: docs/superpowers/specs/2026-08-09-research-followup.md）：TODOboard P0/P1 改进（getOverdueTodos 死代码接线 + Agent 级 0time bug + repeat/cron）、ToolAgent 确认（2 真主体+6 引擎+轻量注册）、安全分类器（Claude Code auto 机制全貌 + CoBeing 落地建议 fail-closed）、GUI 入口收敛（折叠菜单+advancedNav 开关）；决策表全更新（✅4 📌5 🔬1）④新增 start-core-sim.ts（隔离陈默数据根启动验证 gateway 挂载）
- [Change] 12 条决策落地（2026-08-09）：①**投票完全删除**（用户：此前声称删但未真删）——vote/ 3 文件 + agent.ts 注册 + runtime 实例化/双全局别名（含 __cobeing 命名空间占位）+ index 导出 + 2 测试文件断言 + GUI 教程文案，build 通过 + 66 files/588 tests 全绿 + 真实验证 9/9（源码/dist 无残留、Runtime 正常构造无 vote 别名）+ dist 残留清理；②**experience-reflect 旧工具完全删除**（tools/ 文件 + agent.ts 注册，新系统 reflectFromHistory 不受影响）；③12 条决策全文记录到当前待办.md「决策记录」表（✅2 📌5 🔬2），报告顶部标注 4 条被否决建议（群组并发/HRR/TODO 隔离/Docker 沙箱替换为安全分类器），核心技术.md 第 4 条状态机建议标注已放弃（代码自 06-09 即无状态机，原为过期文档）
- [Change] 第一性原理调研报告（docs/information/2026-08-08-research-cobeing-first-principles.md）：29 外部来源 + 代码一手事实。7 项设计获主流验证（管家入口拓扑/文件记忆/权限控制平面/prompt 协作策略/群组组织感/Market 分级/防御工程）；12 项需修正（管家职责软约束实测失守→结构约束、群组多 Agent 并发执行证据不支持、TODO 六账本→单一真相源、HRR 死代码桩、状态机方向建议撤销、记忆纪律、投票过度设计、ToolAgent 注册、experience-reflect 冗余、Docker 沙箱负担、GUI 六入口张力、LLMGateway 名实不符）。结论：自研编排理性，多 Agent 降级为产品叙事与路由配置；P0/P1/P2 建议与当前待办形成闭环
- [Debug] 修复陈默验证发现的 3 项核心问题（66 files/589 tests 全绿 + 实测）：①**OOM 根因**——conversation-loop 工具执行 `workingDir ?? process.cwd()` 静默兜底项目根 → grep 全盘扫描 4GB 堆耗尽；改为 workingDir 缺失时 fail-fast 拒绝工具 + 明确错误（新增 conversation-loop.test.ts 2 测试）②**传值保护**——wake-system 两处 `getGroup?.()?.effectiveWorkspace` 兜底 agent.effectiveWorkspace，杜绝 undefined 泄漏 ③**扫描上限**——grep/glob 文件数 5000→2000、深度 20→12、grep 单文件 2MB 跳过 + 截断提示（异常场景实测 21s→2.4s 无 OOM）④**path-guard 增强**——增加 data-sim-chenmo 段 + .bak 段名拦截（+4 测试）。真实 E2E 早晨场景 S1-S3 全绿无回归
- [Change] 陈默模拟用户真实验证专项（3 轮迭代后全绿）：path-guard 工具链（新增 detectDataPathMisuse，read/write/edit/glob/grep 5 工具接入）**35/35 误用路径拦截 + 正常路径回归全过**；管家 JOB.md "工作类任务一律派发 + 用户个人事务走本地 md" 纪律生效（S4b/S5/S8/S9/S11/S15 均派发群组）；dataRoot 传递链修复；conversation-loop 网络错误识别扩展。用户视角 E2E 27 断言 26 过 + 脚本缺陷修复后复验全绿（晨间 md/TODOboard 语义/设计群组/采买真实查价/创作群组/Market 门禁/出行群组/经验归属全链路真实运转）。发现并处置：**core OOM 崩溃**（备份目录同级干扰管家 grep 扫描 + 4GB 堆上限 → 备份移 docs/archive/ 重启稳定，建议 start-core 提高堆上限）；**测试脚本会话锁缺陷**（管家 [已停止] 占位回复被误当成功 → 新增 sendAndWaitButler 60s 重试修复）；S4a 断言过严修正。数据保留（data-sim-chenmo/ 4 群组 9 Agent 供用户观察）。低风险项：网络错误分支无测试覆盖、glob ** 重复（既有）、bash 降级无 containment（既有）
- [Change] 多轮测试全量验证（先展示计划获批后执行，5 层全通过）：全量 574 tests 全绿（64 files）+ 后端 7 包/GUI 构建通过 + 冒烟 19/19、25/25 + 真实 E2E 两轮——首轮 DeepSeek 偶发 terminated 断连（已知环境问题）重试后管家直做完成（76 工具事件 + 自校验，产物 pvz_demo.html 21KB）；用户批准加跑群组版后 **10/10 断言全绿**（创建 2 Agent → 建群 → 全局 TODO + 回执 6 次 → 派发 → @唤醒 → 群内协作 → 群组工作区 index.html 31KB + UI-INTERFACE.md）。发现项：E2E 脚本 verifyArtifacts 强制断言建群组、与管家合理直做决策不符（请求未要求群组时）；smoke 不加载 .env 属已知降级。测试后按上次流程清理：删测试 Agent/群组/观测/日志、registry 恢复核心（仅 butler/host）、任务状态三文件重置 `[]`
...
- [Change] 收尾：测试数据全面清空（删测试 Agent 前端工程师/游戏开发工程师、归档/观测数据、错误 CWD 残留目录、registry 崩溃备份、重置管家任务状态、删 666MB .tmp-screens 截图——node rmSync 中文路径静默崩溃改 bash rm）+ 推送到 GitHub preview（添加 remote https://github.com/CH3SH-LC/CoBeing.git，153 项变更提交 b2cb380，force-with-lease 覆盖 08-01 旧快照；大文件超限问题通过清理 .tmp-screens 解决；data/ 恢复纯净系统核心）
